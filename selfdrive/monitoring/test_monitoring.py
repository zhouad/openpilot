...
TEST_TIMESPAN = 180  # seconds
DISTRACTED_SECONDS_TO_ORANGE = dm_settings._DISTRACTED_TIME - dm_settings._DISTRACTED_PROMPT_TIME_TILL_TERMINAL + 15
DISTRACTED_SECONDS_TO_RED = dm_settings._DISTRACTED_TIME + 15
INVISIBLE_SECONDS_TO_ORANGE = dm_settings._AWARENESS_TIME - dm_settings._AWARENESS_PROMPT_TIME_TILL_TERMINAL + 15
INVISIBLE_SECONDS_TO_RED = dm_settings._AWARENESS_TIME + 15
...
def make_msg(face_detected, distracted=False, model_uncertain=False):
  ...
  ds.leftDriverData.leftBlinkProb = 0.  # Always not distracted
  ds.leftDriverData.rightBlinkProb = 0.  # Always not distracted
...
class TestMonitoring:
  ...
  def test_normal_driver(self):
    ds_vector = [msg_ATTENTIVE] * int(TEST_TIMESPAN / DT_DMON)
  ...
  def test_sometimes_transparent_commuter(self):
    events, _ = self._run_seq(always_no_face, always_false, always_true, always_false)
    self._assert_no_events(events)
